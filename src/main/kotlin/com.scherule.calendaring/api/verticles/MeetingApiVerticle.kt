...
@Inject constructor(
    ...
    companion object {
        ...
        internal val CREATE_MEETING_SERVICE_ID = "createMeeting"
        internal val GET_MEETING_SERVICE_ID = "getMeeting"
    ...
    override fun start() {
        ...
        vertx.eventBus().consumer<JsonObject>(CREATE_MEETING_SERVICE_ID).toObservable()
                .subscribe { message ->
                    Observable.just(objectMapper.readValue(message.body().getJsonObject("body").encode(), Meeting::class.java))
                            .flatMapSingle(meetingService::createMeeting)
                            .subscribe(
                                    { message.reply(objectMapper.writeValueAsString(it),
                                            DeliveryOptions().addHeader("content-type", "application/json")) },
                                    { manageError(message, it, "createMeeting") }
                            )
                }

        vertx.eventBus().consumer<JsonObject>(GET_MEETING_SERVICE_ID).toObservable()
                .subscribe { message ->
                    val meetingId = message.body().getString("meetingId")
                    Observable.just(meetingService.getMeeting(meetingId))
                            .subscribe(
...
//        vertx.eventBus().consumer<JsonObject>(GET_MEETING_SERVICE_ID).handler { message ->
//            try {
//                val meetingId = message.body().getString("meetingId")
